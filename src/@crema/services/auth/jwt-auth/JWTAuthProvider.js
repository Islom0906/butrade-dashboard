...
const JWTAuthAuthProvider = ({children}) => {
  ...
  jwtAxios.interceptors.response.use(
      (res) => res,
      async (err) => {
        const getRefToken = localStorage.getItem('butradeRefToken')
        ...
        if (err.response.status === 401 && err.response.config.method !== "get" && getRefToken) {
          try {
            ...
            localStorage.setItem('butradeToken', newToken.access)
  ...
  useEffect(() => {
    const getAuthUser = () => {
      const token = localStorage.getItem('butradeToken');
      if (!token) {
        setJWTAuthData({
          user: undefined,
          isLoading: false,
  ...
  const signInUser = async ({username, password}) => {
    if (localStorage.getItem('butradeRefToken')){
      localStorage.removeItem('butradeRefToken')
    }

    dispatch({type: FETCH_START});
    try {
      const {data} = await jwtAxios.post(`/users/user/token/`,{username, password});
      localStorage.setItem('butradeToken', data.access);
      localStorage.setItem('butradeRefToken', data.refresh);
  ...
  const logout = async () => {
    localStorage.removeItem('butradeToken');
    localStorage.removeItem('butradeRefToken');
    // setAuthToken();
    setJWTAuthData({
      user: null,
